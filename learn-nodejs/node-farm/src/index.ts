...
const overviewTemp = fs.readFileSync(
	path.join(__dirname, "..", "templates", "overview-template.html"),
	"utf-8"
);
const cardTemp = fs.readFileSync(
	path.join(__dirname, "..", "templates", "card-template.html"),
	"utf-8"
);
...
const replace = (temp: string, el: any) => {
	let out = temp.replace(/{%PRODUCT_NAME%}/g, el.productName);
	...
	if (!el.organic) out = out.replace(/{%NOT_ORGANIC%}/g, "not-organic");
